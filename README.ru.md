...
Это идентификатор *приложения*, а не аккаунта — своя пара нужна каждому MTProto-клиенту.
Занимает минуту:

1. Открой https://my.telegram.org и введи номер телефона в международном формате (`+971…`).
2. Код подтверждения придёт **внутрь Telegram**, сообщением от официального сервисного аккаунта, —
   не по SMS. Введи его на странице.
3. Нажми **API development tools**.
4. Заполни форму. Обязательны только *App title* и *Short name*; короткое имя — от 5 до 32
   символов, буквы и цифры. URL можно оставить пустым, платформу выбрать `Desktop` (или `Other`) —
   на работу приложения это никак не влияет.
5. Нажми **Create application**. Страница покажет `App api_id` (число) и `App api_hash`
...
- Пара переиспользуется: одно приложение годится для любого аккаунта, под которым ты входишь, и
  на любом количестве машин. Заводить второе незачем.
- `api_hash` держи в секрете. Сам по себе доступа к аккаунту он не даёт, но Telegram может
  забанить `api_id`, замеченный в спаме, а удалить созданное приложение нельзя. Ради этого `.env`
  и в `.gitignore`.
- `my.telegram.org` открывается не из всех сетей. Если страница не грузится или форма отдаёт
  ошибку при отправке — попробуй другую сеть или браузер, сайт заметно капризный.
...
| `send_file` / `download_media` | отправить локальный файл или URL; скачать вложение, в том числе пересланную сторис |
